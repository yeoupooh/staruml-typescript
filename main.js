...
//     return TypeScriptCodeGenerator.generate(base, path, options).then(
//       result.resolve,
//       result.reject
//     );
    ...
    FileSystem.showOpenDialog(
      ...
      function (err, files) {
        if (!err) {
          if (files.length > 0) {
            path = files[0];
            generateTypeScript(base, path, options).then(
              result.resolve,
              result.reject
            );
          } else {
...
function handleConfigure() {
  CommandManager.execute(
    Commands.FILE_PREFERENCES,
    TypeScriptConfigure.getId()
  );
}
...
function about() {
  window.alert(
    "This is a code generator for typescript which is made by Thomas Min."
  );
}

function init() {
  app.commands.register("ts:helloworld", handleShowMessage);
  app.commands.register("ts:generate", handleGenerate);
  app.commands.register("ts:config", handleConfigure);
  app.commands.register("ts:about", about);
}
